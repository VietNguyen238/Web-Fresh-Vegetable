...
export default function Button({ className, children, widthIcon = true, background = 'bg-primary', ...rest }: Props) {
  return (
    <button
      className={`${background} $ hover:opacity-90 text-sm hover:shadow-md group ease duration-300 px-[24px] py-[10px] rounded-full text-wrap text-white flex items-center gap-2 ${className}`}
      {...rest}
    >
